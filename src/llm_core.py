...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt DarkMind' canonical OpenAI-style messages to native Ollama /api/chat.

    DarkMind carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/pewdiepie-archdaemon/darkmind")
        h.setdefault("X-OpenRouter-Title", "DarkMind")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip DarkMind-only metadata before sending messages to providers.
