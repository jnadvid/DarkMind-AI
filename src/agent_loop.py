...
Streaming agent loop for darkmind-ui.
...
# DarkMind-AI is a Spanish-first assistant: every user-facing reply and every
# research output MUST be written in Castilian Spanish (español de España),
# regardless of the language the user writes in.
_LANGUAGE_DIRECTIVE = """\
## Idioma / Language
- SIEMPRE respondes en español (castellano de España), sin importar en qué idioma te escriba el usuario. Toda respuesta visible al usuario y toda investigación se redactan en español.
- Usa un tono natural y correcto en castellano. No traduzcas literalmente: escribe como un hablante nativo.
- Mantén en su idioma original los nombres propios, comandos, rutas de archivo, fragmentos de código y nombres de herramientas; el texto explicativo va en español.
- ALWAYS reply in Spanish (Castilian) no matter what language the user writes in. This is a hard requirement for DarkMind-AI."""

_AGENT_PREAMBLE = """\
You are DarkMind-AI, an AI assistant with tool access. You can run shell commands, execute Python, search the web, \
...
```
GENERIC LOOPBACK to ANY DarkMind internal endpoint. Use this whenever the user wants something the UI can do but there's NO named tool for it. Every UI button hits some /api/* endpoint — you can hit the same one. Auth is handled automatically.
...
def _assemble_prompt(tool_names: set, disabled_tools: set = None, compact: bool = False) -> str:
    ...
    if compact:
        ...
        parts = [
            "You are DarkMind-AI, an AI assistant with tool access.",
