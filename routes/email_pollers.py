                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in DarkMind (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in DarkMind" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "darkmind_kind" if "darkmind_kind" in cols else "'scheduled' AS darkmind_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                darkmind_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-DarkMind-Origin"] = "darkmind-ui"
                outer["X-DarkMind-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", darkmind_kind or "scheduled")[:64]
                outer["X-DarkMind-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `darkmind-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `DARKMIND_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `darkmind-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("DARKMIND_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `DARKMIND_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (DARKMIND_INPROCESS_POLLERS=0); "
            "drive `darkmind-mail poll-scheduled` externally."
        )
        return
    import asyncio
