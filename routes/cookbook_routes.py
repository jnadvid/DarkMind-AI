...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[darkmind] HF token: applied"; '
    'else '
    'echo "[darkmind] HF token: NOT SET — gated/private models will be denied. '
    'Add one in DarkMind Settings -> Cookbook -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            proc = await asyncio.create_subprocess_exec(
                ssh_keygen, "-t", "ed25519", "-N", "", "-C", "darkmind-cookbook", "-f", str(key_path),
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When DarkMind runs from a venv (e.g. native macOS install), put its bin
        ...
        if remote and is_windows:
            ...
            ps_lines = []
            ps_lines.append('$sessionDir = "$env:TEMP\\darkmind-sessions"')
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if host:
            ...
            # LOCAL scan: use sys.executable (the venv Python DarkMind is already
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines = ["#!/bin/bash"]
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('DARKMIND_PREFLIGHT_EXIT=""')
            # Put DarkMind's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('    DARKMIND_PREFLIGHT_EXIT=127')
                runner_lines.append('  fi')
                runner_lines.append('fi')
            elif "ollama" in req.cmd:
                ...
                runner_lines.append(f'DARKMIND_OLLAMA_HOST={_bash_squote(_ollama_host)}')
                runner_lines.append(f'DARKMIND_OLLAMA_PORT="{_ollama_port}"')
                runner_lines.append('DARKMIND_OLLAMA_URL=""')
                runner_lines.append('for _ody_ollama_try in $(seq 1 20); do')
                runner_lines.append('  for _ody_ollama_port in "$DARKMIND_OLLAMA_PORT" 11434; do')
                ...
                runner_lines.append('        DARKMIND_OLLAMA_URL="$_ody_ollama_url"')
                runner_lines.append('        DARKMIND_OLLAMA_PORT="$_ody_ollama_port"')
                runner_lines.append('        break 3')
                runner_lines.append('      fi')
                runner_lines.append('    done')
                runner_lines.append('  done')
                runner_lines.append('  [ "$_ody_ollama_try" -eq 1 ] && echo "[darkmind] Waiting for an existing Ollama API on ports ${DARKMIND_OLLAMA_PORT}/11434..."')
                runner_lines.append('  sleep 1')
                runner_lines.append('done')
                runner_lines.append('if [ -n "$DARKMIND_OLLAMA_URL" ]; then')
                runner_lines.append('  if [ "$DARKMIND_OLLAMA_PORT" != "' + _ollama_port + '" ]; then')
                runner_lines.append('    echo "[darkmind] Selected Ollama port ' + _ollama_port + ' was not reachable; using running Ollama on port ${DARKMIND_OLLAMA_PORT}."')
                runner_lines.append('  fi')
                runner_lines.append('  echo "[darkmind] Ollama API ready on port ${DARKMIND_OLLAMA_PORT}: ${DARKMIND_OLLAMA_URL}"')
                runner_lines.append('  echo "[darkmind] This task is monitoring an existing Ollama server; stopping it here will not stop an external Docker/system service."')
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('if ! command -v ollama &>/dev/null; then')
                runner_lines.append('  echo "ERROR: Ollama not found and no Ollama API is reachable on 127.0.0.1, localhost, or host.docker.internal (ports ${DARKMIND_OLLAMA_PORT}/11434)."')
                ...
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('DARKMIND_OLLAMA_URL="http://${DARKMIND_OLLAMA_HOST}:${DARKMIND_OLLAMA_PORT}"')
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[darkmind] WARNING: remote Ollama will bind to ${DARKMIND_OLLAMA_HOST}:${DARKMIND_OLLAMA_PORT} so DarkMind can reach it from this host."')
                    runner_lines.append('echo "[darkmind] Ollama has no built-in authentication; expose this only on a trusted LAN/VPN or provide an explicit OLLAMA_HOST with your own access controls."')
                runner_lines.append('echo "Starting ollama server on ${DARKMIND_OLLAMA_HOST}:${DARKMIND_OLLAMA_PORT}..."')
                runner_lines.append('OLLAMA_HOST="${DARKMIND_OLLAMA_HOST}:${DARKMIND_OLLAMA_PORT}" ollama serve')
                ...
                runner_lines.append('  DARKMIND_PREFLIGHT_EXIT=1')
                runner_lines.append('fi')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if platform == "windows":
            ...
            setup_script = (
                'powershell -Command "'
                "New-Item -ItemType Directory -Force -Path $env:TEMP\\darkmind-sessions | Out-Null; "
