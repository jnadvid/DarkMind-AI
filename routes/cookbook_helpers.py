...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When DarkMind runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends DarkMind' own interpreter directory to PATH. If DarkMind itself is
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'DARKMIND_USER_SHELL="${SHELL:-}"',
        'if [ -n "$DARKMIND_USER_SHELL" ] && [ -x "$DARKMIND_USER_SHELL" ]; then',
        '  DARKMIND_USER_PATH="$("$DARKMIND_USER_SHELL" -ic \'printf "__DARKMIND_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__DARKMIND_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$DARKMIND_USER_PATH" ]; then export PATH="$DARKMIND_USER_PATH:$PATH"; fi',
        'fi',
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by DarkMind.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$DARKMIND_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $DARKMIND_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        runner_lines.append('  exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('  exit "$DARKMIND_PREFLIGHT_EXIT"')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('DARKMIND_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $DARKMIND_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $DARKMIND_CMD_EXIT ==="; exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('echo ""; echo "=== Process exited with code $DARKMIND_CMD_EXIT ==="')
        runner_lines.append('exit "$DARKMIND_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[darkmind] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('      _darkmind_has_cudart() {')
    ...
    runner_lines.append('      }')
    runner_lines.append('      if _darkmind_has_cudart; then')
    runner_lines.append('        echo "[darkmind] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('        echo "[darkmind] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[darkmind]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[darkmind]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    else')
    runner_lines.append('      echo "[darkmind] WARNING: no HIP/CUDA toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[darkmind]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[darkmind]   Install ROCm for AMD GPUs or vLLM/CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
...
def _llama_cpp_rebuild_cmd() -> str:
    ...
    return (
        ...
        'echo "[darkmind] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\darkmind-sessions"
