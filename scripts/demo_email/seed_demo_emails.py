...
This populates the `demo@darkmind.local` Dovecot account (which has NO mbsync
...
USER = os.getenv("DEMO_IMAP_USER", "demo@darkmind.local")
...
LOOKUP_MSGID = "<demo-lookup-deepseek@darkmind.local>"
...
def _msg(*, frm, to=None, subject, text, html=None, days_ago=0, hours_ago=0,
    ...
    m["Message-ID"] = msg_id or make_msgid(domain="darkmind.local")
    if in_reply_to:
...
def build_dataset() -> list[dict]:
    ...
    add("INBOX", "", _msg(
        ...
        text=("Hey,\n\nSaw your work on the DarkMind stack — seriously impressive. "
        ...
        html=("<p>Hey,</p><p>Saw your work on the <b>DarkMind</b> stack — seriously "
    ...
    add("INBOX", "", _msg(
        frm="Greg <greg@darkmind-demo.example>",
    ...
    orig_id = make_msgid(domain="darkmind.local")
    add("Sent", "(\\Seen)", _msg(
    ...
    add("INBOX", "(\\Flagged)", _msg(
        frm="Ops Bot <ops@darkmind-demo.example>",
        subject="[URGENT] prod is on fire 🔥 — darkmind-ui 502s",
        days_ago=0, hours_ago=1,
        text=("PAGE: darkmind-ui is returning 502s on the /api/chat endpoint.\n"
...
def _seed_cache() -> None:
    ...
    reply = (
        ...
        "— drafted for you by your DarkMind assistant"
    )
    ...
    try:
        ...
        con.execute(
            ...
            (LOOKUP_MSGID, "", "INBOX", "quick q for the slide — DeepSeek-V3 param count?",
             "greg@darkmind-demo.example", summary, "demo", now))
        con.commit()
...
def _wipe(conn: imaplib.IMAP4) -> int:
    ...
    """
    safe_target = USER.endswith("@darkmind.local") or HOST in ("localhost", "127.0.0.1", "::1")
...
def main() -> int:
    ...
    try:
        ...
        print(f"seeded {len(items)} demo message(s) into {USER} "
              f"(INBOX + Sent). Switch to the 'Demo' account in DarkMind to view.")
        return 0
    finally:
        try:
            conn.logout()
