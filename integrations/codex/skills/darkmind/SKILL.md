...
Use this skill when a user asks to interact with DarkMind from Codex.
...
- `DARKMIND_API_TOKEN`: Scoped API token created in DarkMind Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in DarkMind Settings and expose both values to the terminal session.
...
- All DarkMind data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
If you are about to reach the DarkMind host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass DarkMind Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.
