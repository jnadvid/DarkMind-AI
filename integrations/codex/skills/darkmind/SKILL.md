---
name: darkmind
description: Use when the user asks Codex to read or write DarkMind data from a terminal Codex session through the scoped Codex Agent API. Requires DARKMIND_URL and DARKMIND_API_TOKEN.
---

# DarkMind-AI

Usa esta habilidad cuando un usuario pida interactuar con DarkMind-AI desde Codex.
...
- `DARKMIND_API_TOKEN`: Token de API con ámbito creado en Configuración de DarkMind-AI > Integraciones > Añadir integración > Agente Codex.

Si falta alguno de los dos valores, no adivines las credenciales. Indica al usuario que cree un token de Agente Codex en la Configuración de DarkMind-AI y exponga ambos valores a la sesión de terminal.
...
- Todo acceso a datos de DarkMind-AI DEBE pasar por la API HTTP con ámbito bajo `/api/codex/*`.
...
La API de Codex admite tareas pendientes/listas de verificación:
...
```bash
python3 integrations/codex/scripts/darkmind_api.py capabilities
python3 integrations/codex/scripts/darkmind_api.py todos list
python3 integrations/codex/scripts/darkmind_api.py todos add "Follow up"
```
...
```bash
python3 integrations/codex/scripts/darkmind_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
La API de Codex admite lecturas de correo con ámbito:
...
```bash
python3 integrations/codex/scripts/darkmind_api.py emails list 5
python3 integrations/codex/scripts/darkmind_api.py emails read UID
```

Si `/api/codex/capabilities` no muestra `email.read: true`, no inspecciones el correo. Pide al usuario que habilite la lectura de correo en la configuración del Agente Codex.
...
```bash
python3 integrations/codex/scripts/darkmind_api.py GET /api/codex/memory
python3 integrations/codex/scripts/darkmind_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
Si estás a punto de acceder al host/contenedor de DarkMind-AI, importar internos de la aplicación, consultar la base de datos o llamar directamente a módulos de ayuda de MCP, detente. Esas rutas omiten la Configuración de DarkMind-AI y los ámbitos de los tokens. Pide al usuario que habilite el interruptor de herramienta del Agente Codex correspondiente.
