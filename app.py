...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. DarkMind's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-DarkMind-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-DarkMind-Owner") or "").strip()
            ...
            if auth_header.startswith("Bearer dmd_"):
                raw_token = auth_header[7:]
                # Sanity check: tokens are "dmd_" + 43 chars of base64
...
async def _startup_event():
    ...
    # `DARKMIND_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("DARKMIND_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (DARKMIND_INPROCESS_TASKS=0); "
