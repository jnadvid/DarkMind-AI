// ============================================
// DarkMind UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__darkmindDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'DarkMind Chat';
  ...
  if (_opener) window._darkmindRouteOpener = _opener;
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'darkmind-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message DarkMind...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      const tipEl = el('welcome-tip');
      incognitoBtn.title = chk.checked ? 'Desactivar modo incógnito' : 'Activar modo incógnito — sin memoria, sin historial guardado';
      const welcomeName = document.querySelector('.welcome-name');
      if (chk.checked) {
        incognitoBtn.innerHTML = INCOGNITO_EYE_CLOSED + '<span class="incognito-label">Incógnito</span>';
        if (welcomeName) {
          welcomeName.dataset.originalHtml = welcomeName.innerHTML;
          welcomeName.innerHTML = '<svg class="welcome-boat" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><line x1="8" y1="16" x2="16" y2="8"/><line x1="8" y1="8" x2="16" y2="16"/></svg>Incógnito';
        ...
        incognitoBtn.innerHTML = INCOGNITO_EYE_OPEN + '<span class="incognito-label">Incógnito</span>';
  ...
  const UI_VIS_KEY = 'darkmind-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'darkmind-toolbar-visibility';
    try {
...
// ============================================
function startDarkMindApp() {
  if (window.__darkmindAppStarted) return;
  window.__darkmindAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('darkmind-doc-open-' + _curSession) === '1') {
  ...
  window._darkmindBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._darkmindRouteOpener) {
          try { window._darkmindRouteOpener(); } catch (_) {}
          window._darkmindRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startDarkMindApp, { once: true });
} else {
  startDarkMindApp();
}
