...
export async function loadPersonalDocs() {
  ...
  try {
    ...
    if (files.length === 0) {
      ...
      placeholder.textContent = 'Arrastra archivos arriba para añadir a RAG';
    ...
    error.textContent = 'Error al cargar los archivos';
...
async function _deleteFile(filepath, displayName) {
  if (!await uiModule.styledConfirm(`¿Eliminar "${displayName}" de RAG?`, { confirmText: 'Eliminar', danger: true })) return;
  try {
...
export async function uploadRagFiles(fileList) {
  ...
  try {
    ...
    const data = await res.json();
    if (zone) zone.textContent = 'Arrastra archivos aquí o haz clic para subir';
    await loadPersonalDocs();
    return data;
  } catch (e) {
