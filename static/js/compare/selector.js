...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'darkmind-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('darkmind-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('darkmind-compare-synth-' + state._compareMode);
