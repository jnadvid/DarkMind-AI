...
function _persistSelections() {
  if (state._selectedModels.length > 0) {
    Storage.setJSON('darkmind-compare-selections-' + (state._compareMode || 'chat'), state._selectedModels);
  }
  if ((state._compareMode === 'search' || state._compareMode === 'research') && state._searchSynthModels) {
    Storage.setJSON('darkmind-compare-synth-' + state._compareMode, state._searchSynthModels);
  }
}
...
const POOL_STORAGE_KEY = 'darkmind-shuffle-pool-excluded';
