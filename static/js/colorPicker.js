...
const LS_RECENT = 'darkmind-recent-colors';
const MAX_RECENT = 12;
...
function computeSuggestions() {
  ...
  return [
    { hex: hsvToHex(_h + 180, _s, _v),                                   label: 'Complementario' },
    { hex: hsvToHex(_h + 30, _s, _v),                                    label: 'Análogo +30°' },
    { hex: hsvToHex(_h - 30, _s, _v),                                    label: 'Análogo -30°' },
    { hex: hsvToHex(_h + 150, _s, _v),                                   label: 'Complemento dividido' },
    { hex: hsvToHex(_h, _s, clamp(_v > 50 ? _v - 30 : _v + 30, 10, 95)), label: 'Variación de tono' },
  ];
}
...
function buildPopover() {
  ...
  p.innerHTML = `
    ...
    <div class="cp-row">
      ...
      <button class="cp-eyedropper" title="Cuentagotas" type="button">
    ...
    <div class="cp-section-label">Sugerencias</div>
...
function syncUI() {
  ...
  rContainer.innerHTML = recs.length
    ...
    : '<div class="cp-recent-empty">(ninguno aún)</div>';
}
...
function wireHandlers(p) {
  ...
  if (window.EyeDropper) {
    ...
    eye.title = 'Cuentagotas no compatible con este navegador';
  }
}
