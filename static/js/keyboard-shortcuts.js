...
export function initKeyboardShortcuts(modules) {
  ...
  window._darkmindKeybinds = { ..._defaultKeybinds };

  // Load saved keybinds
  fetch('/api/auth/settings', { credentials: 'same-origin' })
    .then(r => r.json())
    .then(s => { if (s.keybinds) window._darkmindKeybinds = { ..._defaultKeybinds, ...s.keybinds }; })
    .catch(() => {});
  ...
  document.addEventListener('keydown', (e) => {
    const kb = window._darkmindKeybinds;
      ...
      uiModule.styledConfirm('¿Eliminar esta sesión?', { confirmText: 'Eliminar', danger: true }).then(ok => {
        ...
        fetch(`${API_BASE}/api/session/${sid}`, { method: 'DELETE' }).then(async () => {
          ...
          if (nextSession) {
            ...
            el('current-meta').textContent = 'DarkMind Chat';
