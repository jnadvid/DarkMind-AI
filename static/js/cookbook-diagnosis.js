...
function _diagFixIcon(label) {
  ...
  if (l.startsWith('retry') || l.startsWith('reintentar') || l.includes('relaunch') || l.includes('relanzar') || l.includes('restart') || l.includes('reiniciar')) {
  ...
  if (l.startsWith('kill') || l.startsWith('stop') || l.startsWith('matar') || l.startsWith('detener')) {
    return _svg('<rect x="6" y="6" width="12" height="12" rx="1"/>');
  }
  if (l.startsWith('switch') || l.startsWith('cambiar') || l.startsWith('elegir') || l.includes('use ')) {
...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    message: 'SGLang necesita una GPU/acelerador visible en este servidor.',
    suggestion: 'Acción sugerida: cambia esta configuración de servicio a llama.cpp para servir en CPU/local, o elige un servidor con GPU.',
    fixes: [
      { label: 'Cambiar a llama.cpp', action: (panel) => _openCpuServeEdit(panel) },
      { label: 'Elegir servidor con GPU', action: (panel) => _openServeEditFromDiagnosis(panel) },
    ],
  },
  {
    pattern: /flashinfer.*version.*does not match|flashinfer-cubin version/i,
    message: 'Incompatibilidad de versión de FlashInfer.',
    fixes: [
      { label: 'Corrección automática: omitir verificación de versión', action: (panel) => _serveAutoFix(panel, 'FLASHINFER_DISABLE_VERSION_CHECK=1'), autofix: true },
      { label: 'Corrección correcta: pip install versión compatible', action: () => {} },
    ],
  },
  {
    pattern: /torch\.cuda\.is_available\(\).*False|No CUDA runtime/i,
    message: 'vLLM necesita una GPU CUDA/ROCm visible.',
    suggestion: 'Acción sugerida: cambia esta configuración de servicio a llama.cpp para servir en CPU/local, o elige un servidor con GPU.',
    fixes: [
      { label: 'Cambiar a llama.cpp', action: (panel) => _openCpuServeEdit(panel) },
      { label: 'Elegir servidor con GPU', action: (panel) => _openServeEditFromDiagnosis(panel) },
  ...
  },
  {
    ...
    message: 'El motor vLLM no pudo iniciarse. Comprueba el error anterior.',
    fixes: [
      { label: 'Reintentar con --enforce-eager', action: (panel) => _serveAutoRetry(panel, '--enforce-eager'), autofix: true },
      { label: 'Reintentar con contexto 4096', action: (panel) => _serveAutoRetry(panel, '--max-model-len 4096'), autofix: true },
      { label: 'Reducir contexto a 4096', action: (panel) => _setPanelField(panel, 'ctx', '4096') },
      { label: 'Reducir GPU mem a 0.80', action: (panel) => _setPanelField(panel, 'gpu_mem', '0.80') },
    ],
  },
  {
    pattern: /weight_loader.*unexpected keyword|Unexpected key.*state_dict/i,
    message: 'Formato de modelo incompatible con esta versión de vLLM.',
    fixes: [
      { label: 'Probar con trust remote code', action: (panel) => _setPanelCheckbox(panel, 'trust_remote', true) },
    ],
  },
  {
    pattern: /enable-auto-tool-choice requires --tool-call-parser/i,
    message: 'La selección automática de herramientas necesita un analizador de llamadas a herramientas.',
    fixes: [
      { label: 'Reintentar con --tool-call-parser hermes', action: (panel) => _serveAutoRetry(panel, '--tool-call-parser hermes'), autofix: true },
    ],
  },
  {
    pattern: /Please pass.*trust.remote.code=True|contains custom code which must be executed to correctly load/i,
    message: 'El modelo requiere código personalizado. Activa --trust-remote-code.',
    fixes: [
      { label: 'Reintentar con --trust-remote-code', action: (panel) => _serveAutoRetry(panel, '--trust-remote-code'), autofix: true },
    ],
  },
  {
    pattern: /does not recognize this architecture|model type.*but Transformers does not/i,
    message: 'Arquitectura del modelo demasiado nueva para la versión instalada de vLLM/transformers.',
    fixes: [
      { label: 'Probar con --trust-remote-code', action: (panel) => _serveAutoRetry(panel, '--trust-remote-code'), autofix: true },
      { label: 'Actualizar vLLM en el servidor', action: (panel) => {
  ...
  },
  {
    ...
    message: 'Incompatibilidad de paquetes transformers/kernels.',
    fixes: [
      { label: 'Reparar paquete kernel', action: (panel) => {
        const taskEl = panel.closest('.cookbook-task');
        const task = taskEl ? _loadTasks().find(t => t.sessionId === taskEl.dataset.taskId) : null;
        const host = task?.remoteHost || '';
        const prefix = _buildEnvPrefix();
  ...
  },
  {
    ...
    message: 'Ollama no está instalado en este servidor. Ejecuta: curl -fsSL https://ollama.com/install.sh | sh',
    fixes: [
      { label: 'Copiar comando de instalación', action: () => _copyText('curl -fsSL https://ollama.com/install.sh | sh') },
    ],
  },
  {
    pattern: /llama-server.*command not found|llama\.cpp.*not found|No module named.*llama_cpp|No module named 'starlette_context'/i,
    message: 'El servidor llama-cpp-python no está instalado. Ejecuta: pip install "llama-cpp-python[server]"',
    fixes: [
      { label: 'Abrir Dependencias', action: () => _openCookbookDependencies('llama_cpp') },
      { label: 'Copiar comando de instalación', action: () => _copyText('pip install "llama-cpp-python[server]"') },
    ],
  },
  {
    pattern: /CUDA Toolkit not found|Unable to find cudart library|missing:\s*CUDA_CUDART/i,
    message: 'llama.cpp encontró nvcc, pero falta la biblioteca de tiempo de ejecución CUDA.',
    suggestion: 'Acción sugerida: relanza con el ejecutor actualizado para que llama.cpp compile solo en CPU, o instala un toolkit/runtime CUDA completo en este servidor para llama.cpp con GPU.',
    fixes: [
      { label: 'Editar servicio', action: (panel) => _openServeEditFromDiagnosis(panel) },
      { label: 'Abrir Dependencias', action: () => _openCookbookDependencies('llama_cpp') },
    ],
  },
  {
    ...
    message: 'El servicio de difusión necesita PyTorch y diffusers. Instala diffusers desde Cookbook → Dependencias.',
    fixes: [
      { label: 'Abrir Dependencias', action: () => _openCookbookDependencies('diffusers') },
      { label: 'Copiar comando de instalación', action: () => _copyText('python3 -m pip install "diffusers[torch]"') },
    ],
  },
  {
    pattern: /Triton kernels.*Failed to import|cannot import name '\w+' from 'triton_kernels/i,
    message: 'Incompatibilidad de versión de los kernels Triton. Advertencia no crítica — el modelo seguirá ejecutándose, pero sin los kernels MoE optimizados.',
    fixes: [
      { label: 'Actualizar triton en el servidor', action: (panel) => {
        const taskEl = panel.closest('.cookbook-task');
        const task = taskEl ? _loadTasks().find(t => t.sessionId === taskEl.dataset.taskId) : null;
        const host = task?.remoteHost || '';
        const prefix = _buildEnvPrefix();
  ...
  },
  {
    ...
    message: 'Disco lleno en el servidor. Libera espacio antes de volver a intentarlo.',
    fixes: [
      { label: 'Comprobar tamaño de caché HF', action: (panel) => _runQuickCmd(panel, 'du -sh ~/.cache/huggingface 2>/dev/null') },
    ],
  },
  {
    pattern: /Connection refused|Could not connect|Connection reset by peer/i,
    message: 'Falló la conexión de red. El servidor puede estar inaccesible o HuggingFace está caído.',
    fixes: [
      { label: 'Probar conectividad con HF', action: (panel) => _runQuickCmd(panel, 'curl -sI https://huggingface.co 2>&1 | head -3') },
    ],
  },
  {
    pattern: /attention_sink|sliding.window.*not supported|sliding_window.*incompatible/i,
    message: 'El modelo usa funciones de atención no compatibles con esta versión de vLLM.',
    fixes: [
      { label: 'Actualizar vLLM en el servidor', action: (panel) => {
        const taskEl = panel.closest('.cookbook-task');
        const task = taskEl ? _loadTasks().find(t => t.sessionId === taskEl.dataset.taskId) : null;
  ...
  },
  {
    ...
    message: 'Traceback de Python detectado — puede ser un error gestionado; revisa los registros.',
    fixes: [
      { label: 'Matar procesos vLLM', action: (panel) => _runQuickCmd(panel, 'pkill -f vllm') },
    ],
  },
];
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## DarkMind Cookbook troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',
...
export function _showDiagnosis(panel, diagnosis, sourceText) {
  ...
  if (task?.type === 'serve' && task.payload?._cmd && !fixes.some(f => f.label === 'Editar servicio')) {
    fixes.push({ label: 'Editar servicio', action: (p) => _openServeEditFromDiagnosis(p) });
  }
  const suggestionText = diagnosis.suggestion || (fixes.length
    ? `Acción sugerida: ${fixes[0].label}.`
    : 'Acción sugerida: copia el error y ajusta la configuración de servicio.');
...
export async function _runQuickCmd(panel, cmd) {
  ...
  const diag = panel.querySelector('.cookbook-diagnosis');
  if (diag) { diag.classList.remove('hidden'); diag.textContent = `Ejecutando: ${fullCmd}...`; }
  ...
  try {
    ...
    if (diag) diag.textContent = res.ok ? `Listo: ${cmd}` : `Falló (HTTP ${res.status})`;
  } catch (e) {
    if (diag) diag.textContent = `Error: ${e.message}`; // 'Error:' kept as-is (technical prefix)
  }
}
