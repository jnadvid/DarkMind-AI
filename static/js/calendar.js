...
const BADGE_SEEN_KEY = 'darkmind-calendar-badge-seen';
...
function _wireAll(body) {
  ...
  try {
    ...
    if (calBody && splitter) {
      ...
      if (!alreadySet) {
        const saved = parseInt(localStorage.getItem('darkmind.cal.detailH') || '0', 10);
      ...
      const onUp = () => {
        ...
        if (px) { try { localStorage.setItem('darkmind.cal.detailH', String(px)); } catch {} }
      };
      const onDown = (ev) => {
        ev.preventDefault();
        dragging = true;
      ...
      const resetSplit = () => {
        ...
        try { localStorage.removeItem('darkmind.cal.detailH'); } catch {}
      };
  ...
  body.querySelectorAll('[data-date]').forEach(cell => {
    ...
    cell.addEventListener('drop', async (e) => {
      ...
      _render();
      uiModule.showToast?.('Movido', { duration: 4000, action: 'Deshacer', actionHint: 'Ctrl+Z', onAction: _popAndRunCalUndo });
    });
  });
}
...
function _showEventForm(existing, defaultDate, defaultEndDate) {
  ...
  body.innerHTML = `<div class="cal-form cal-form-bespoke${_expandedAtStart ? ' is-expanded' : ''}">
    <button type="button" class="cal-form-mobile-cancel" id="cal-form-mobile-cancel" title="Cancel" aria-label="Cancelar evento">
    ...
    <div class="cal-form-details" id="cal-form-details" aria-hidden="${_expandedAtStart ? 'false' : 'true'}">
      ...
      <div class="cal-loc-row">
        ...
        <a id="cal-f-loc-map" class="cal-loc-map" href="#" target="_blank" rel="noopener noreferrer" title="Open in Maps" aria-label="Abrir en Apple Maps" tabindex="-1">
...
const LS_KEY = 'darkmind-calendar-cache';
