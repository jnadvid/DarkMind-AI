...
const GROUP_STATE_KEY = 'darkmind-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('darkmind-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('darkmind-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
