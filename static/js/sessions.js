...
let _sortMode = Storage.get('darkmind-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'DarkMind Chat';
...
const FOLDER_STATE_KEY = 'darkmind-folder-state';
const FOLDER_ORDER_KEY = 'darkmind-folder-order';
...
function buildFolderSubmenu(sessionId, currentFolder, dropdown) {
  ...
  moveItem.innerHTML = '<span class="dropdown-icon">' + _folderIcon + '</span><span>Mover a carpeta</span>';
  ...
  noneOpt.textContent = '(Sin carpeta)';
  ...
  newOpt.addEventListener('click', async (e) => {
    e.stopPropagation();
    const name = await styledPrompt('Nombre para esta carpeta:', {
      title: 'Nueva carpeta',
      placeholder: 'p. ej. Trabajo, Investigación, Borradores',
      confirmText: 'Crear',
    });
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to DarkMind kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'DarkMind Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('darkmind-doc-open-' + id) === '1';
...
export async function materializePendingSession() {
  ...
  const name = isIncognito ? 'Incognito' : `${base} ${new Date().toLocaleTimeString()}`;
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('darkmind-session-sort', mode);
  else Storage.remove('darkmind-session-sort');
  renderSessionList();
}
