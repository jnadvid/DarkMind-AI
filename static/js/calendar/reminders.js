...
function _formatReminderBody(note) {
  ...
  if (dtstartRaw) {
    ...
    if (!isNaN(start.getTime())) {
      ...
      if (mins >= 1 && mins <= 60) return `Empieza en ${mins} min (${when}${when2})`;
      if (mins === 0) return `Empieza ahora (${when}${when2})`;
      if (mins > 60) {
        const h = Math.round(mins / 60);
        return `Empieza en ${h} hora${h === 1 ? '' : 's'} (${when}${when2})`;
      }
      if (mins >= -60) return `Empezó hace ${Math.abs(mins)} min (${when}${when2})`;
      return `Estaba programado para ${when}${when2}`;
    }
  }
  ...
  try {
    ...
    for (const note of notes) {
      ...
      fetch(`${API_BASE}/api/notes/fire-reminder`, {
        ...
        body: JSON.stringify({
          note_id: note.id,
          title: note.title || 'Recordatorio de calendario',
          body,
        }),
      }).catch(() => {});
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification(note.title || 'Recordatorio de calendario', {
          body,
          icon: '/static/favicon.png',
          tag: `cal-remind-${note.id}`,
        });
      }
      if (uiModule.showToast) uiModule.showToast((note.title || 'Recordatorio de calendario') + (body ? ' — ' + body : ''));
    }
