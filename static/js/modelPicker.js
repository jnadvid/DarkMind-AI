...
const RECENT_KEY = 'darkmind-model-recent';
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('darkmind-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('darkmind-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('darkmind:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('darkmind:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `darkmind-model-favorites[0]`
