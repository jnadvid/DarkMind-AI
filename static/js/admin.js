...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('darkmind:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('#adm-add-api, #adm-add-local').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'darkmind.addModels.' + sec.id + '.open';
    let open = false;
...
async function loadMcpServers() {
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-mcp-delete]').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!await uiModule.styledConfirm('¿Eliminar este servidor MCP?', { confirmText: 'Eliminar', danger: true })) return;
...
async function loadWebhooks() {
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-wh-delete]').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!await uiModule.styledConfirm('¿Eliminar este webhook?', { confirmText: 'Eliminar', danger: true })) return;
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'darkmind_backup.json';
