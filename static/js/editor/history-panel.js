...
export function createHistoryPanel({ undo, redo }) {
  ...
  function refreshHistoryPanelIfOpen() {
    ...
    for (let i = 0; i < state.undoStack.length; i++) {
      ...
      rows.push({ offset: -(state.undoStack.length - i), label: s._label || 'Edición', ts: s._ts });
    }
    rows.push({ offset: 0, label: 'Actual', ts: Date.now(), current: true });
    for (let i = state.redoStack.length - 1; i >= 0; i--) {
      const s = state.redoStack[i];
      rows.push({ offset: (state.redoStack.length - i), label: s._label || 'Edición', ts: s._ts, future: true });
    }
