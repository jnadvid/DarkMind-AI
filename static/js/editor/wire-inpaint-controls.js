...
export function wireInpaintControls({
  ...
  document.getElementById('ge-mask-vis')?.addEventListener('click', () => {
    ...
    btn.innerHTML = `${state.maskVisible ? EYE_OPEN_SM : EYE_OFF_SM}<span id="ge-mask-vis-label">${state.maskVisible ? 'Ocultar' : 'Mostrar'}</span>`;
    btn.title = state.maskVisible ? 'Ocultar máscara' : 'Mostrar máscara';
  ...
  document.getElementById('ge-inpaint-invert')?.addEventListener('click', () => {
    ...
    uiModule.showToast('Máscara invertida');
  });
