...
export function createAdjPopupSystem({ composite, saveState, renderLayerPanel }) {
  ...
  function openFxPopup(layer, anchorEl) {
    ...
    const items = [
      { type: 'brightness-contrast', label: 'Brillo / Contraste' },
      { type: 'hue-saturation',      label: 'Tono / Saturación' },
      { type: 'levels',              label: 'Niveles' },
      { type: 'color-balance',       label: 'Balance de color' },
    ];
  ...
  function openAdjPopup(layer, type, anchorEl, existingAdj) {
    ...
    <div class="ge-adj-foot">
      ...
      <button class="ge-btn ge-btn-sm ge-btn-primary ge-adj-apply-btn" data-adj-action="ok">Aplicar</button>
    </div>
    `;
  ...
  function buildAdjBody(layer, type, body, popEl) {
    ...
    const sliderRow = (key, label, min, max, value, suffix) => `
      <div class="ge-adj-row" data-adj-key="${key}">
        ...
        <button class="ge-adj-revert" type="button" title="Restablecer este control" data-revert-key="${key}">${revertIcon}</button>
      </div>
    `;
    if (type === 'brightness-contrast') {
      ...
      body.innerHTML = `
      ${sliderRow('brightness', 'Brillo',    -100, 100, bSlider, '')}
      ${sliderRow('contrast',   'Contraste', -100, 100, cSlider, '')}
    `;
    } else if (type === 'hue-saturation') {
      const hSlider = Math.round(p.hue);
      const sSlider = Math.round((p.saturation - 1) * 100);
      body.innerHTML = `
      ${sliderRow('hue',        'Tono',       -180, 180, hSlider, ' °')}
      ${sliderRow('saturation', 'Saturación', -100, 100, sSlider, '')}
    `;
    } else if (type === 'levels') {
      ...
      <details class="ge-adj-hist-details"${isMobile ? '' : ' open'}>
        <summary>Histograma</summary>
        <div class="ge-adj-hist-wrap">
          <canvas class="ge-adj-histogram" width="280" height="80"></canvas>
          <div class="ge-adj-hist-handles">
            <div class="ge-adj-hist-handle hist-h-black"  data-handle="inBlack"  title="Negro de entrada — arrastra"></div>
            <div class="ge-adj-hist-handle hist-h-gamma"  data-handle="gamma"    title="Gamma — arrastra"></div>
            <div class="ge-adj-hist-handle hist-h-white"  data-handle="inWhite"  title="Blanco de entrada — arrastra"></div>
          </div>
        </div>
      </details>
      ${sliderRow('inBlack',  'Negro entrada',  0, 254, p.inBlack, '')}
      ${sliderRow('inWhite',  'Blanco entrada', 1, 255, p.inWhite, '')}
      ${sliderRow('gamma',    'Gamma',          10, 990, Math.round((p.gamma || 1) * 100), 'γ')}
      ${sliderRow('outBlack', 'Negro salida',   0, 255, p.outBlack, '')}
      ${sliderRow('outWhite', 'Blanco salida',  0, 255, p.outWhite, '')}
    `;
    ...
    `;
      ...
      popEl._cbTone = tone;
      const toneSliders = (t) => `
      ${cbRow(`${t}-r`, '#00d2d2', '#ff5555', 'Cian ↔ Rojo',       p[t].r)}
      ${cbRow(`${t}-g`, '#d855d8', '#55d855', 'Magenta ↔ Verde',   p[t].g)}
      ${cbRow(`${t}-b`, '#e6e64a', '#4a78ff', 'Amarillo ↔ Azul',  p[t].b)}
    `;
      body.innerHTML = `
      <div class="ge-adj-cb-tone-picker">
        <select class="ge-adj-cb-tone-select">
          <option value="shadows"${tone === 'shadows' ? ' selected' : ''}>Sombras</option>
          <option value="midtones"${tone === 'midtones' ? ' selected' : ''}>Medios tonos</option>
          <option value="highlights"${tone === 'highlights' ? ' selected' : ''}>Iluminaciones</option>
        </select>
      </div>
