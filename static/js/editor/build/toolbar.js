...
export function buildToolbar({ currentTool, onSelectTool, onClearSelection }) {
  ...
  const tools = [
    ...
    { sep: true },
    { id: 'clone', label: 'Clonar', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="3"/><path d="M9 12l-3 4h12l-3-4"/><path d="M4 20h16"/></svg>', key: 'K' },
    { id: 'lasso', label: 'Lazo', icon: '⟡', key: 'L' },
    { id: 'wand', label: 'Varita', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 4V2"/><path d="M15 16v-2"/><path d="M8 9h2"/><path d="M20 9h2"/><path d="M17.8 11.8L19 13"/><path d="M15 9h0"/><path d="M17.8 6.2L19 5"/><path d="M3 21l9-9"/><path d="M12.2 6.2L11 5"/></svg>', key: 'W' },
    { sep: true },
    ...
    { id: 'rembg', ai: true, label: 'Elim. fondo', icon: '✄' },
    { id: 'sharpen', ai: true, label: 'Enfocar', icon: '◈', key: 'S' },
  ];
  const toolKeyMap = {};
  for (const t of tools) {
    ...
    const clearBadge = (t.id === 'lasso' || t.id === 'wand')
      ? '<span class="ge-tool-clear" title="Limpiar selección" data-clear-tool="' + t.id + '">' +
