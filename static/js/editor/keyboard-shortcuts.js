  ...
  document.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && !isAltGrEvent(e)) {
          ...
          tmp.toBlob(blob => {
            if (blob && navigator.clipboard?.write) {
              navigator.clipboard.write([new ClipboardItem({ 'image/png': blob })]).then(() => {
                uiModule.showToast(isCut ? 'Cortado al portapapeles' : 'Copiado al portapapeles');
              }).catch(() => uiModule.showToast(isCut ? 'Cortado (solo en el editor)' : 'Copiado (solo en el editor)'));
            }
          }, 'image/png');
          if (isCut) {
          ...
          layer.canvas.toBlob(blob => {
            if (blob && navigator.clipboard?.write) {
              navigator.clipboard.write([new ClipboardItem({ 'image/png': blob })])
                .then(() => uiModule.showToast('Capa copiada al portapapeles'))
                .catch(() => uiModule.showToast('Error al copiar (¿permiso de portapapeles denegado?)'));
            }
          }, 'image/png');
          return;
        }
      ...
      if (e.altKey && e.key === 'a' && state.imgWidth > 0 && state.imgHeight > 0) {
        ...
        composite();
        drawLassoOverlay();
        uiModule.showToast('Todo seleccionado — Ctrl+C para copiar, Del para eliminar');
      }
      // Ctrl+V handled by the paste event listener.
