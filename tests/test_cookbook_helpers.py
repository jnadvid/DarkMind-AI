...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "DARKMIND_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $DARKMIND_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$DARKMIND_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_darkmind_has_cudart' in script
    ...
    assert script.index('_darkmind_has_cudart') < script.index('DGGML_CUDA=ON')
